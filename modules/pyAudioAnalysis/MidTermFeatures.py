...
def mid_feature_extraction(signal, sampling_rate, mid_window, mid_step,
    ...
    for i in range(n_feats):
        ...
        mid_feature_names[i + 2*n_feats] = short_feature_names[i] + "_" + "max"
        mid_feature_names[i + 3*n_feats] = short_feature_names[i] + "_" + "skewness"
        mid_feature_names[i + 4*n_feats] = short_feature_names[i] + "_" + "kurtosis"
        ...
        while cur_position < num_short_features:
            ...
            mid_features[i + 2*n_feats].append(np.amax(cur_st_feats))
            mid_features[i + 3*n_feats].append(skew(cur_st_feats))
            mid_features[i + 4*n_feats].append(kurtosis(cur_st_feats))
